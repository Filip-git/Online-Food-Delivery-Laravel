...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `name`  AS SELECT `administrator`.`ID` AS `ID`, `administrator`.`IDRestorana` AS `IDRestorana`, `administrator`.`IDMusterije` AS `IDMusterije`, `administrator`.`IDKorisnika` AS `IDKorisnika` FROM `administrator` ;
...
--
ALTER TABLE `korisnik`
  MODIFY `ID` int(50) UNSIGNED NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=43;
